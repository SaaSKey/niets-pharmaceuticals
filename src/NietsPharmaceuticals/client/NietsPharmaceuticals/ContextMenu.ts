...
onFillWorldObjectContextMenu.addListener(
  (
    playerNum: number,
...
function doEquipHammer(player: IsoPlayer, hammer: HandWeapon) {
  ISTimedActionQueue.add(
    new ISEquipWeaponAction(player, hammer, 50, true, false)
  );
}
  ...
  (playerNum: number, context: ISContextMenu, items: any[]) => {
    ...
    if (hammers.length > 0) {
      ...
      hammers.forEach((hammer: InventoryItem) => {
        const option = hammersContext.addOption(
          hammer.getDisplayName(),
          player,
          doEquipHammer,
          hammer
        );
        ...
        const conditionPercent = luautils.round(
          (hammer.getCondition() / hammer.getConditionMax()) * 100,
          2
        );
